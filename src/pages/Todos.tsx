...
import { FaEdit, FaTrash, FaCheck, FaClock, FaSearch, FaChevronLeft, FaChevronRight } from "react-icons/fa";
import { useAuth } from "../contexts/AuthContext";
import { Todo, QueryParams, User } from "../types";
import type { PaginationResponse } from "../api/todos";
...
const Todos: React.FC<TodosProps> = () => {
  ...
  const { data, isLoading, error } = useQuery<PaginationResponse<Todo>, Error>({
    queryKey: ["todos", page, itemsPerPage, search, status, category, priority, isAuthenticated],
    queryFn: () => {
      const params: QueryParams = { page, limit: itemsPerPage, search, status, category, priority };
  ...
  const completedCount = data?.data?.filter((todo: Todo) => todo.completed).length || 0;
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div>
                <h1 className="text-4xl font-bold text-gray-900 mb-2">
                  {isAuthenticated ? `${(user as User)?.name}'s Tasks` : 'Todo Application'}
                </h1>
              ...
              <div className="grid grid-cols-2 md:grid-cols-2 gap-4 md:flex md:gap-8">
                ...
                <div className="text-center md:text-right">
                  ...
                  <p className="text-3xl font-bold text-green-600" aria-label={`Completed tasks: ${completedCount}`}>
                    {completedCount}
                  </p>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-1 flex-wrap justify-center max-w-2xl">
                      ...
                      {data.totalPages && Math.max(1, Math.min(data.totalPages - 6, page - 3)) > 1 && (
                        <span className="px-2 py-2 text-gray-500">...</span>
                      )}

                      {data.totalPages && Array.from({ length: Math.min(7, data.totalPages - 1) }, (_, i) => {
                        const startPage = Math.max(2, Math.min(data.totalPages - 6, page - 3));
                      ...
                      {data.totalPages && Math.min(data.totalPages - 6, page - 3) + 7 < data.totalPages && (
                        <span className="px-2 py-2 text-gray-500">...</span>
                      )}
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (pageNum >= 1 && pageNum <= (data.totalPages || 1)) {
                            ...
                            alert(`Please enter a page number between 1 and ${data.totalPages || 1}`);
                          }
                        }}
